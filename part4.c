...
{
	// setting the pointer stucture x value to the value of x passed in
	pt->x = x;
	// setting the pointer structure y value to the value of y passed in
	pt->y = y;
...
{
	// grab the difference of the pointer structure x values and pow them by 2
	double x = pow((*pt1).x - (*pt2).x,2);
	// grab the difference of the pointer structure y values and pow them by 2
	double y = pow((*pt1).y - (*pt2).y,2);
	// get your x and y value and add them together to then square root them
